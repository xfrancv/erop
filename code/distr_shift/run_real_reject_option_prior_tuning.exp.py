...
``--auto-target-prior [N_MIN]`` replaces all the knobs with the autonomous
search of ``tasks/target_label_prior_selection.md``
(:mod:`prior_shift.target_prior_search`): it guarantees at least ``N_MIN``
evaluation examples (default 300), picks the least-identifiable class pair
and the skew/rest split maximising the expected epistemic-vs-total
separation, and confirms the winner with a short MCMC validation gate before
...
from prior_shift.target_prior_search import (
...
def validate_auto_candidate(P, y, train_prior, cand, n_va, n_eval, loss,
    ...
    ``n_va`` examples, MCMC, evaluation on ``n_eval`` examples) at the
    candidate prior and checks the operational symptoms the flip-test proxies
    predict:

    - the identifiability warning fires on the chosen pair (the split is
      genuinely weakly identified, so epistemic uncertainty exists);
    - the mean epistemic uncertainty is above ``epi_threshold`` and a
      non-negligible portion of eval examples carries it;
    - the total- and epistemic-ranking AuRC-regrets differ beyond the trial
      noise (the two reject-option predictors actually behave differently).

    Returns ``(ok, report_lines)``.
    """
    all_idx = np.arange(len(y))
    diffs = np.zeros(trials)
    epi_mean = np.zeros(trials)
    portion = np.zeros(trials)
    ...
    for t in range(trials):
        ...
        cond_risk = bayes_post @ loss.T
        h = cond_risk.argmin(axis=1)
        total = cond_risk.min(axis=1)
        epi = total - aleatoric
        h_true = bayes_decision(
            corrected_posterior(post_ev, train_prior, cand.alpha), loss)
        losses_b, losses_ref = loss[h, y_ev], loss[h_true, y_ev]
        _, reg_total = selective_curves(losses_b, losses_ref, total)
        _, reg_epi = selective_curves(losses_b, losses_ref, epi)
        diffs[t] = reg_total.mean() - reg_epi.mean()
        epi_mean[t] = epi.mean()
        portion[t] = (epi >= epi_threshold).mean()

    gap, sem = diffs.mean(), diffs.std() / np.sqrt(trials)
    checks = {
        "ident warning fires on the pair": fired.mean() >= 0.5,
        "epistemic mass non-negligible":
            epi_mean.mean() > epi_threshold and portion.mean() >= 0.05,
        "regret-AuRC gap beyond noise": abs(gap) > max(sem, 1e-4),
    }
    lines = [f"  validate ({cand.describe()}): "
             f"ident fired {fired.sum()}/{trials}, "
             f"mean epi {epi_mean.mean():.4f}, "
             f"portion>={epi_threshold:g} {portion.mean():.2f}, "
             f"regret-AuRC gap (total-epi) {gap:+.4f} (sem {sem:.4f})"]
    failed = [name for name, ok in checks.items() if not ok]
    lines.append("  -> PASS" if not failed
...
def main() -> None:
    ...
    parser.add_argument(
        "--auto-target-prior", type=int, nargs="?", const=300, default=None,
        metavar="N_MIN",
        help="Select the target prior autonomously (see "
             "tasks/target_label_prior_selection.md): guarantee at least "
             "N_MIN evaluation examples (default 300) and maximise the "
             "epistemic-vs-total separation. Mutually exclusive with "
             "--test-prior; overrides the pair/ratio knobs.")
    ...
    auto_note = None
    if args.auto_target_prior is not None:
        if args.test_prior is not None:
            sys.exit("error: --auto-target-prior and --test-prior are "
                     "mutually exclusive")
        if (args.confusable_pair is not None
                or args.pair_rest_ratio is not None
                or tuple(args.pair_ratio) != (1.0, 7.0)):
            print("warning: --auto-target-prior overrides --confusable-pair / "
                  "--pair-ratio / --pair-rest-ratio")
        # The flip test scores the same calibrated posteriors the MCMC will
        # see; a biased calibration poisons both, so the search must not run.
        ratio = bundle.get("marginal_ratio")
        if ratio is None:
            print("warning: this bundle predates the calibration-consistency "
                  "check; the auto search cannot detect calibration bias, "
                  "which misleads the flip test and the MCMC alike. Consider "
                  "retraining with run_base_predictor_exp.py (BCTS).")
        else:
            ratio = np.asarray(ratio, dtype=float)
            if np.any((ratio > CALIB_RATIO_THRESHOLD)
                      | (ratio < 1 / CALIB_RATIO_THRESHOLD)):
                sys.exit("error: the base model failed its calibration "
                         "consistency check (see the warning above); the "
                         "auto search would inherit the same biased "
                         "posteriors. Retrain with run_base_predictor_exp.py "
                         "or pass an explicit --test-prior.")
        n_adapt_wanted = max(args.sizes) if args.sweep else args.n_test
        try:
            search = search_target_prior(
                P, y_pool, train_prior,
                n_adapt=n_adapt_wanted, n_min=args.auto_target_prior,
                loss=loss, registry_pair=pair_idx,
                adjacent_pairs_only="ordinal" in spec.tags,
                class_names=class_names)
        except ValueError as e:
            sys.exit(f"error: {e}")
        print("\n".join(search.report_lines))
        if not search.gate_passed:
            sys.exit("error: auto target-prior search aborted (gate failed, "
                     "see above)")
        if not search.ranked:
            sys.exit("error: auto target-prior search found no feasible "
                     "candidate; reduce N_MIN or the adaptation size")

        # Step-4 validation gate: confirm the winner with the real pipeline,
        # falling back to the runners-up.
        n_va = min(n_adapt_wanted,
                   sorted(args.sizes)[len(args.sizes) // 2] if args.sweep
                   else args.n_test)
        gate_rng = np.random.default_rng(args.seed + 1)
        chosen = None
        for cand in search.ranked[:3]:
            ...
            if ok:
                ...
                break
        if chosen is None:
            chosen = search.ranked[0]
            print("warning: no candidate passed the validation gate; using "
                  "the best-scored one anyway -- the epistemic-vs-total "
                  "contrast may be weak on this dataset")
        target_prior = chosen.alpha
        pair_idx, pair_source = chosen.pair, "auto search"
        rest = ("keep" if chosen.pair_rest_ratio is None else
                f"{chosen.pair_rest_ratio[0]:g}:{chosen.pair_rest_ratio[1]:g}")
        auto_note = (f"N_min={args.auto_target_prior}, {chosen.describe()}, "
                     f"guaranteed n_eval>={chosen.n_eval}, "
                     f"E={chosen.E:.3f} A={chosen.A:.3f} G={chosen.G:.4f} "
                     f"S={chosen.S:.2f}, recommended n_test<="
                     f"{search.recommended_n_test or 'unbounded'}")
    elif args.test_prior is not None:
    ...
    save_run_args(
        ...
        extra={
            ...
            **({"auto_target_prior": auto_note} if auto_note else {}),
        },
