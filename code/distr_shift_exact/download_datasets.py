"""Download the eight datasets of README S7 into ``data/``.

Fashion-MNIST (Zalando IDX), CIFAR-10 / CIFAR-100 (fast.ai PNG-folder mirror)
and the five MedMNIST v2 ``.npz`` archives from Zenodo. Everything uses the
standard library plus NumPy -- no torch/torchvision/medmnist.

Run with::

    python download_datasets.py                 # all eight
