...
class. It also resolves the README S6.1 splits and reports the two protocol
quantities they determine: the ``m_max <= |eval| / 10`` cap of S6.2, and the
per-class evaluation counts that set how often a query recurs inside its own
adaptation set (S6.3). Reports (and an index) are written to ``data/reports/``
with all images embedded as base64 -- each HTML file is fully self-contained.

Run with::

    python analyze_datasets.py                  # all eight (downloads if missing)
