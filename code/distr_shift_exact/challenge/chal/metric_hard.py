...
GENERATED from ``chal/metric.py`` by ``make_metric_notebook.py --hard-module``;
do not edit. The code is identical; only the docstrings differ, because Kaggle
renders them to competitors and the hard variant does not say that batches
differ in their label prior (tasks/hard_variant.md). What follows is the easy
variant's description, kept for the organisers.
