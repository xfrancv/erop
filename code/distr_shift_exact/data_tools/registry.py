"""Static metadata for each dataset: download URLs and class names.

The split *policy* is uniform across datasets in this project (README S6.1) --
the official train split is the development set, and the official test split
(with val merged in when the dataset has one) is the evaluation set -- so
unlike the sibling ``distr_shift`` project there is no per-dataset override.
...
DATASETS: dict[str, DatasetSpec] = {
    ...
    "tissuemnist": DatasetSpec(
        ...
        description=(
            ...
            "natural weakly-identifiable pair."
        ),
        confusable_pair=(
            "Glomerular endothelial cells",
            "Interstitial endothelial cells",
